...
# Expressões aritméticas e funções de agregação
...
```{sql connection=con}
SELECT PNAME, ACRES AS AREA_ATUAL, ACRES * 1.20 AS NOVA_AREA FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES AS AREA_ATUAL, ACRES / 3.0 AS AREA_AJUSTADA FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE AS TAXA_ATUAL, (FEE + 50.00) / 2 AS TAXA_AJUSTADA FROM PRESERVE
```

4.  Apresente os valores médio, máximo e mínimo de ACRES para todas as reservas naturais localizadas no Arizona.

```{sql connection=con}
SELECT AVG(ACRES) AS MEDIA, MAX(ACRES) AS MAXIMO, MIN(ACRES) AS MINIMO FROM PRESERVE
WHERE STATE = 'AZ'
```
...
*Resposta:* Existe 1 taxa de entrada que não seja igual a 0 na tabela `PRESERVE`.

7.  Escreva uma instrução SELECT que demonstre que PNAME não contém atualmente valores duplicados.

```{sql connection=con}
SELECT COUNT(PNAME) AS PNAME_TOTAL, COUNT(DISTINCT PNAME) AS PNAME_DISTINTOS FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(0.02 * ACRES) AS NOVA_TAXA_MÉDIA_AZ FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT COUNT(DNO) AS FUNC_DNO20 FROM EMPLOYEE
WHERE DNO = 20
```
*Resposta:* Três funcionários trabalham no Departamento 20 (Moe, Curly e George).
...
```
*Resposta:* Três departamentos possuem funcionários (os departamentos 10, 20 e 40).
